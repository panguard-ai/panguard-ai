...
const incidentBadgeLabels: Record<IncidentStatus, string> = {
  resolved: "Resolved",
...
function UptimeChart({ service, index }: { service: Service; index: number }) {
  return (
          ...
          {service.uptimeHistory.map((day, i) => {
            ...
            return (
              <div
                ...
                title={`Day ${30 - i}: ${cfg.label}`}
              />
            );
          })}
        </div>
        <div className="flex items-center justify-between mt-3">
          <span className="text-text-tertiary text-xs">30 days ago</span>
          <span className="text-text-secondary text-xs font-medium">
            30-day uptime: {service.uptime.toFixed(2)}%
          </span>
    ...
    <FadeInUp delay={index * 0.05}>
      <div className="bg-surface-1 border border-border rounded-xl p-5">
        <div className="flex flex-wrap items-center gap-3 mb-3">
          ...
          >
            {incidentBadgeLabels[incident.status]}
          </span>
        </div>
    ...
    <>
      ...
      <SectionWrapper spacing="spacious">
        <SectionTitle
          ...
          title="Panguard AI Service Status"
          subtitle="Real-time operational status and uptime history for all Panguard AI platform services."
        />
      </SectionWrapper>
      ...
      <SectionWrapper>
        <FadeInUp>
          ...
          <p className="text-text-secondary text-sm mb-6">
            All services monitored in real time across global regions.
          </p>
        </FadeInUp>

        <div>
          {services.map((service, idx) => (
            <ServiceRow key={service.name} service={service} index={idx} />
          ))}
        </div>
      </SectionWrapper>
      ...
      <SectionWrapper dark>
        <FadeInUp>
          ...
          <p className="text-text-secondary text-sm mb-8">
            Visual representation of service availability over the past 30 days.
          </p>
        </FadeInUp>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {topServices.map((service, idx) => (
            <UptimeChart key={service.name} service={service} index={idx} />
          ))}
        </div>
        ...
        <FadeInUp delay={0.2}>
          <div className="flex items-center justify-center gap-6 mt-8">
            <div className="flex items-center gap-2">
              ...
              <span className="text-text-tertiary text-xs">Operational</span>
            </div>
            <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-text-tertiary text-xs">Outage</span>
            </div>
          </div>
        </FadeInUp>
      </SectionWrapper>
      ...
      <SectionWrapper>
        <FadeInUp>
          ...
          <p className="text-text-secondary text-sm mb-8">
            Past incidents and scheduled maintenance events with resolution
            details.
          </p>
        </FadeInUp>
        ...
        <FadeInUp>
          <div className="text-center max-w-xl mx-auto">
            ...
            <p className="text-text-secondary mt-3 leading-relaxed text-sm">
              Subscribe to receive instant notifications when service status
              changes. Stay informed about scheduled maintenance and incident
              resolutions.
            </p>

...
export CORS_ALLOWED_ORIGINS=https://dashboard.panguard.ai  # 即將推出
```
...
docker run -d \
  ...
  -e CORS_ALLOWED_ORIGINS=https://dashboard.panguard.ai \  # 即將推出
  panguard/manager
```
</CodeGroup>
...
<Info>
`ALLOW_ANONYMOUS_UPLOAD=true` 讓使用者無需 API 金鑰即可提交威脅資料。上傳的資料仍會經過驗證和聲譽評分。
</Info>
...
services:
  manager:
    ...
    environment:
      ...
      - CORS_ALLOWED_ORIGINS=https://dashboard.panguard.ai  # 即將推出

---
title: "Quick Start"
description: "Install the CLI, scan your system, and start real-time protection in under 5 minutes. 100% open source, no account required."
icon: "rocket"
---

From zero to full protection in 4 steps. No account, no login, no configuration files. Works on macOS, Linux, and Windows.

<Steps>
  <Step title="Install the CLI">
    Choose your platform and install Panguard with a single command.

    <Tabs>
      <Tab title="macOS / Linux">
        ```bash
        curl -fsSL https://get.panguard.ai | bash
        ```
      </Tab>
      <Tab title="Windows">
        ```powershell
        powershell -ExecutionPolicy Bypass -Command "irm https://get.panguard.ai/windows | iex"
        ```
      </Tab>
      <Tab title="npm (all platforms)">
        ```bash
        npm install -g @panguard-ai/panguard
        ```
      </Tab>
    </Tabs>

    Verify the installation:

    ```bash
    panguard --version
    # Expected: 0.3.3
    ```

    <Tip>
      **Intel Mac users:** The curl installer downloads the ARM64 binary. Either install via npm
      (`npm install -g @panguard-ai/panguard`) or enable Rosetta 2 first
      (`softwareupdate --install-rosetta`) before using the curl installer.
    </Tip>
  </Step>

  <Step title="Connect your AI agents">
    Auto-configure all detected AI platforms with a single command:

    ```bash
    panguard setup
    ```

    This detects and configures Claude Code, Cursor, QClaw, OpenClaw, Codex CLI, Claude Desktop, and more automatically. Each platform gets MCP or native skill integration so your AI agents can use Panguard security tools directly.

    ```
      PANGUARD AI - Setup

      Detected platforms:
        Claude Code    ~/.claude/settings.local.json
        Cursor         ~/.cursor/mcp.json
        OpenClaw       ~/.openclaw/skills/panguard/SKILL.md

      All platforms configured successfully.
    ```
  </Step>

  <Step title="Run your first scan">
    Execute a quick security audit of your system:

    ```bash
    panguard scan --quick
    ```

    The scan completes in approximately 30 seconds and produces a risk assessment:

    ```
      PANGUARD AI - Security Scanner

      Scanning system environment...

      -- Security Findings ----------------------

      CRITICAL  Port 22 (SSH) exposed to 0.0.0.0
      HIGH      No firewall rules detected
      MEDIUM    Password policy not enforced
      LOW       12 unnecessary services running

      -- Risk Score -----------------------------

      Score: 62/100 [============--------] Grade: C

      -- Recommendations ------------------------

      1. Restrict SSH access to specific IP ranges
      2. Enable and configure system firewall
      3. Enforce password complexity requirements
    ```

    Remove the `--quick` flag for a full scan (~60 seconds) that includes SSL certificate validation, scheduled task audit, and shared folder checks.

    <Tabs>
      <Tab title="Generate PDF report">
        ```bash
        panguard scan --output my-report.pdf
        ```
      </Tab>
      <Tab title="Traditional Chinese output">
        ```bash
        panguard scan --lang zh-TW
        ```
      </Tab>
      <Tab title="Full scan">
        ```bash
        panguard scan
        ```
      </Tab>
    </Tabs>
  </Step>

  <Step title="Start real-time protection">
    A scan is a one-time check. For continuous protection, start the Guard engine with the built-in dashboard:

    ```bash
    panguard guard start --dashboard
    ```

    This starts real-time monitoring and opens a local dashboard in your browser:

    ```
      PANGUARD AI - Guard Engine

      Guard engine starting...

      -- Status ---------------------------------

      Mode:       Learning (Day 1/7)
      Monitoring: processes, network, files
      Rules:      3,760 Sigma rules loaded
      Dashboard:  http://127.0.0.1:9100
      Score:      --/100 (building baseline)

      Guard is now protecting your system.
      Dashboard opened in browser.
    ```

    <Info>
      **How Guard works:** During the first 7 days, the AI observes normal system behavior and builds a baseline (no false positives during this period). Starting on day 8, it automatically detects anomalies, executes responses, and notifies you.
    </Info>
  </Step>
</Steps>

## What to do next

Your system is now scanned and protected with 9,790 detection rules. The dashboard at `http://127.0.0.1:9100` shows real-time status, detected threats, loaded rules, and skill trust status.

<CardGroup cols={2}>
  <Card title="Set up notifications" icon="bell" href="/guides/notifications-setup">
    Receive alerts via Telegram, Slack, Email, or LINE when threats are detected.
  </Card>
  <Card title="Install on OpenClaw & QClaw" icon="bolt" href="/guides/claw-setup">
    One-line setup for OpenClaw (native Skill) and QClaw (MCP).
  </Card>
  <Card title="Generate compliance reports" icon="file-chart-column" href="/guides/compliance-reports">
    Auto-generate ISO 27001, SOC 2, and Taiwan TCSA audit reports.
  </Card>
  <Card title="Explore the CLI" icon="terminal" href="/cli/overview">
    Full reference for every Panguard command and flag.
  </Card>
</CardGroup>

...
<Note>
  **v1.0.0 released** -- Built-in dashboard, OpenClaw & QClaw native support, Skill Auditor as default entry point. [See changelog](/changelog).
</Note>
...
| | |
| --- | --- |
| **10,490+** detection rules | Sigma (4,353) + YARA (6,015) + ATR (61) + community |
| **8** AI platforms | OpenClaw, QClaw, Claude Code, Cursor, Codex CLI, Claude Desktop, Windsurf, WorkBuddy |
...
`panguard setup` auto-detects all AI platforms on your machine (OpenClaw, QClaw, Claude Code, Cursor, etc.) and injects the correct MCP or native Skill configuration for each.
...
<Tabs>
  ...
  <Tab title="QClaw">
    ...
    Registers as an **MCP server** in `~/.qclaw/mcp.json`. Restart QClaw to access 11 security tools.
  </Tab>
  <Tab title="Auto-detect all platforms">
    ...
    Auto-detects and configures all 8 supported AI platforms in one command.
  </Tab>
</Tabs>

<Tip>
  For manual configuration and troubleshooting, see the [OpenClaw & QClaw setup guide](/guides/claw-setup).
</Tip>
...
<CardGroup cols={3}>
  ...
  <Card title="MSPs" icon="server">
    Monitor and protect client endpoints from a single Manager dashboard. Deploy across fleets with centralized policy.
  </Card>
</CardGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="API Reference" icon="code" href="/api-reference/overview">
    REST API docs for Threat Cloud, Manager, and Auth services.
  </Card>

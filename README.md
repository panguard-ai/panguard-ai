...
| **Cisco AI Defense**                    | Full 650+-rule library in skill-scanner production                  | Merged        | [PR #99](https://github.com/cisco-ai-defense/skill-scanner/pull/99)                  |
...
| -------------- | ------------------ | -------------------------------------------------------------------------------------------- |
| **Community**  | $0 forever         | MIT, unlimited self-host, 650+ ATR rules, no signup, no telemetry                             |
...
| ----- | --------------------------- | ------- | ------- |
| **1** | 650+ ATR rules (regex + AST) | < 50ms  | $0      |
...
panguard-ai/
  packages/
    ...
    atr/                     ATR Layer 0: 650+ rules, 10 categories
...
650+ rules total across 10 categories. See [stats.json](https://raw.githubusercontent.com/Agent-Threat-Rule/agent-threat-rules/main/data/stats.json) for the authoritative count.
...
60 days, solo, from zero to: 650+ ATR rules. 6 ecosystem integrations merged. 30+ PRs in flight across Microsoft, Cisco, MISP, OWASP A-S-R-H, NVIDIA, IBM, Meta, OpenSSF, NIST, EU AI Office. Microsoft Copilot SWE Agent wrote regression fixtures presuming ATR coverage; we shipped the matching rules in 2h 16m.

Adam Lin（林冠辛），創辦人，台灣。

跨領域背景：房地產業務、行銷（Threads 累積 3 億次觸及）、嘻哈音樂節製作（第 5 年）、自學工程。

60 天獨自從零開始：650+ 條 ATR 規則、6 個生態系整合 merged、30+ 個 PR 進行中，涵蓋 Microsoft、Cisco、MISP、OWASP A-S-R-H、NVIDIA、IBM、Meta、OpenSSF、NIST、EU AI Office。Microsoft Copilot SWE Agent 主動寫出預期 ATR 偵測的 regression fixture，我們在 2 小時 16 分鐘內 ship 出對應規則。

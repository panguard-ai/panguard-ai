...
const inputPath =
  args.find((a) => a.startsWith('--input='))?.split('=')[1] || join(OUTPUT_DIR, 'scan-full.json');

const IMPACT: Record<string, [string, string]> = {
  'System Prompt Override Attempt': [
    'Can hijack your AI to follow attacker instructions',
    '可以劫持你的 AI，讓它聽攻擊者的話',
  ],
  'Malicious Content in MCP Tool Response': [
    'Hidden commands injected when AI reads tool output',
    'AI 讀取工具回傳時被偷塞指令',
  ],
  'Unauthorized Tool Call Detection': [
    'Secretly calls dangerous tools without asking',
    '偷偷呼叫危險工具完全不問你',
  ],
  'Privilege Escalation and Admin Function Access': [
    'Gains admin access through your AI',
    '透過 AI 取得管理員權限',
  ],
  'Direct Prompt Injection via User Input': [
    'Input tricks your AI into ignoring safety rules',
    '輸入騙 AI 忽略安全規則',
  ],
  'Agent Goal Hijacking Detection': [
    'Redirects your AI to work on attacker goals',
    '把你的 AI 導向攻擊者目標',
  ],
  'Multi-Skill Chain Attack': ['Multiple skills chain together to attack', '多個 skill 聯手攻擊'],
  'Parameter Injection via Tool Arguments': [
    'Injects malicious code through tool parameters',
    '透過工具參數注入惡意程式碼',
  ],
  'Over-Permissioned MCP Skill': ['Requests more permissions than needed', '要的權限比需要的多'],
  'Shell Metacharacter Injection': [
    'Injects shell commands through tool arguments',
    '透過工具參數注入 shell 指令',
  ],
  ...
  'Credential and Secret Exposure': [
    'API keys and passwords can leak through AI',
    'API key 和密碼可能從 AI 洩漏',
  ],
  'Eval Injection': [
    'Executes arbitrary code via eval/Function',
    '透過 eval/Function 執行任意程式碼',
  ],
  'Environment Variable Harvesting': [
    'Extracts all environment variables and secrets',
    '提取所有環境變數和密鑰',
  ],
};
...
const RISK_SUMMARY: Record<string, [string, string]> = {
  CRITICAL: [
    'DANGER: Can control your computer or steal data',
    '危險：可以控制你的電腦或偷你的資料',
  ],
...
function getImpact(title: string): [string, string] {
  ...
  return [
    'Potential security risk — review before installing',
    '偵測到潛在安全風險，安裝前請先審查',
  ];
}

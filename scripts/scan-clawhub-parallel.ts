...
async function scanOne(
  ...
  try {
    ...
    try {
      execSync(`unzip -q -o "${join(wd, 's.zip')}" -d "${join(wd, 'ex')}" 2>/dev/null`, {
        timeout: 5000,
      });
    } catch {
      return mkR(author, slug, downloads, -1, 'NO_CONTENT', 'Unzip failed');
    }
    ...
    return mkR(author, slug, downloads, -1, 'ERROR', String(err));
  } finally {
    if (existsSync(wd)) rmSync(wd, { recursive: true, force: true });
  }
}
...
async function pushTC(results: Result[]) {
  ...
  for (const r of results) {
    ...
    if (r.riskLevel === 'CRITICAL' || r.riskLevel === 'HIGH') {
      promises.push(
        fetch(`${TC_ENDPOINT}/api/skill-threats`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            skillHash: createHash('sha256').update(sn).digest('hex').slice(0, 16),
            skillName: sn,
            riskScore: r.riskScore,
            riskLevel: r.riskLevel,
      ...
      promises.push(
        fetch(`${TC_ENDPOINT}/api/skill-whitelist`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ skillName: sn }),
    ...
    promises.push(
      fetch(`${TC_ENDPOINT}/api/usage`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          event_type: 'scan',
          source: 'bulk-pipeline',
          metadata: { skill: sn, risk: r.riskLevel },
        }),
        signal: AbortSignal.timeout(3000),
      })
        .then(() => {})
...
async function main() {
  ...
  for (const f of ['scan-1000.json', 'scan-10000.json']) {
    ...
    if (existsSync(p)) {
      allResults.push(...(JSON.parse(readFileSync(p, 'utf-8')).results || []));
    }
  }
  // Also load checkpoints > 10000
  const { readdirSync } = await import('node:fs');
  for (const f of readdirSync(OUTPUT_DIR)
    .filter((f) => f.startsWith('scan-checkpoint-'))
    .sort()) {
  ...
  for (let i = startIdx; i < total; i += batchSize) {
    ...
    for (let j = 0; j < batch.length; j += CONCURRENCY) {
      ...
      const results = await Promise.all(
        chunk.map((s: any, idx: number) =>
          scanOne(s.name, s.author, s.downloads, scanContent, atrRules, idx)
        )
      );
      for (const r of results) {
        batchResults.push(r);
    ...
    // Checkpoint
    writeFileSync(
      join(OUTPUT_DIR, `scan-checkpoint-${i}.json`),
      JSON.stringify(
        {
    ...
    for (const r of batchResults)
      if (r.riskScore >= 0) bs[r.riskLevel] = (bs[r.riskLevel] || 0) + 1;
    process.stdout.write('\r'.padEnd(80) + '\r');
    console.log(
      `  Batch ${batchNum} (${i}): ${batchResults.length} new | ${JSON.stringify(bs)} | TC: ${tc.threats}T ${tc.whitelist}W`
    );
  }
    ...
    JSON.stringify(
      {
        ...
        summary: byLevel,
        tcPushed: { threats: totalT, whitelist: totalW },
        results: allResults,
      },
      null,
      2

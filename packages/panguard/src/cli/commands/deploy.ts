...
async function runDeploy(opts: { config?: string; dryRun?: boolean; lang?: string }): Promise<void> {
  // ── Load config ─────────────────────────────────────────
  ...
  planItems.push({
    label: 'AI',
    value: config.ai.preference === 'cloud_ai' ? 'Cloud AI'
  ...
  if (config.notifications.channel !== 'none') {
    ...
    await sleep(500);
    notifSp.succeed(`[${step}/${totalSteps}] ${lang === 'zh-TW' ? '\u901A\u77E5\u7BA1\u9053\u5DF2\u914D\u7F6E' : 'Notification channel configured'}`);
    step++;
  }

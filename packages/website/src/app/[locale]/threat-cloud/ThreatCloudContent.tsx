...
import { useThreatIntelStats } from '@/hooks/useThreatIntelStats';
...
export default function ThreatCloudContent() {
  ...
  const tcStats = useThreatIntelStats();
  ...
  return (
    <>
              ...
              return (
                <FadeInUp key={key} delay={i * 0.08}>
                  <div className="flex gap-6 mb-8 last:mb-0">
                    ...
                    <div className="pb-2 flex-1 bg-surface-1 border border-border rounded-xl px-4 py-3">
      ...
      {/* ── Skill Intelligence ── */}
      <SectionWrapper dark>
        <SectionTitle overline={t('skillIntel.overline')} title={t('skillIntel.title')} />
        <FadeInUp delay={0.05}>
      ...
      <SectionWrapper>
        <SectionTitle
          overline={t('liveIntel.overline')}
          title={t('liveIntel.title')}
          subtitle={t('liveIntel.subtitle')}
        />
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-14 max-w-3xl mx-auto">
          {(
            [
              { key: 'atrRules', value: tcStats.atrRules },
              { key: 'atrPatterns', value: tcStats.atrPatterns },
              { key: 'records', value: tcStats.validatedRecords },
              { key: 'sources', value: tcStats.sources },
            ] as const
          ).map(({ key, value }, i) => (
            <FadeInUp key={key} delay={i * 0.06}>
              <div className="bg-surface-1 rounded-xl border border-border p-5 text-center card-glow">
                ...
                <p className="text-xs text-text-secondary mt-1">{t(`liveIntel.stats.${key}`)}</p>
              </div>
            </FadeInUp>
          ))}
        </div>
        ...
        <div className="grid sm:grid-cols-2 gap-4 mt-6 max-w-3xl mx-auto">
          <FadeInUp delay={0.3}>
            <div className="bg-surface-2 rounded-xl border border-border p-5">
              <p className="text-xs uppercase tracking-wider text-brand-sage font-semibold mb-3">
                {t('liveIntel.badge.promoted')}
              </p>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <p className="text-lg font-bold text-text-primary">{tcStats.autoGeneratedATR}</p>
                  <p className="text-xs text-text-secondary">{t('liveIntel.stats.autoGenATR')}</p>
                </div>
                <div>
                  <p className="text-lg font-bold text-brand-sage">{tcStats.promotedRules}</p>
                  <p className="text-xs text-text-secondary">{t('liveIntel.stats.promoted')}</p>
                </div>
                <div>
                  <p className="text-lg font-bold text-text-secondary">{tcStats.draftRules}</p>
                  <p className="text-xs text-text-secondary">{t('liveIntel.stats.draft')}</p>
                </div>
              </div>
            </div>
          </FadeInUp>
          ...
          <FadeInUp delay={0.35}>
            <div className="bg-surface-2 rounded-xl border border-border p-5">
              <p className="text-xs uppercase tracking-wider text-brand-sage font-semibold mb-3">
                {t('liveIntel.stats.sources')}
              </p>
              <div className="flex flex-wrap gap-1.5">
                {(t.raw('liveIntel.sourceList') as string[]).map((src) => (
                  <span
                    key={src}
                    className="text-[10px] bg-brand-sage/10 text-brand-sage border border-brand-sage/20 rounded-full px-2.5 py-0.5 font-medium"
                  >
                    {src}
                  </span>
                ))}
              ...
              <div className="flex items-center gap-4 mt-4 text-xs text-text-secondary">
                <span>
                  {t('liveIntel.stats.syncInterval')}:{' '}
                  <strong className="text-text-primary">{tcStats.syncInterval}</strong>
                </span>
                <span>
                  {t('liveIntel.stats.lastSync')}:{' '}
                  <strong className="text-text-primary">{tcStats.lastSync}</strong>
                </span>
              </div>
            </div>
          </FadeInUp>
            ...
            <div className="bg-brand-sage/5 border border-brand-sage/20 rounded-xl p-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-4">
                <div className="bg-surface-1 border border-border rounded-lg p-3">
                  <p className="text-xs text-text-secondary">{t('tierNote.freeLabel')}</p>
                  <p className="text-sm font-semibold text-text-primary mt-1">
                    {t('tierNote.freeDesc')}
                  </p>
                </div>
                <div className="bg-surface-1 border border-border rounded-lg p-3">
                  <p className="text-xs text-text-secondary">{t('tierNote.paidLabel')}</p>
        ...
        <div className="text-center max-w-2xl mx-auto">
          ...
          <FadeInUp delay={0.1}>
            <div className="flex flex-wrap justify-center gap-3">
              ...
              <Link
                href="https://docs.panguard.ai/api-reference"

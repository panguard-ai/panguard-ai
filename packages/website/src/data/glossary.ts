  ...
  },
  {
    ...
    longDefinition: {
      ...
      zh: [
        ...
        `PanGuard 的 Wild Scan（2026-04）掃描了 96,096 個 skill，分布：ClawHub (36,378)、OpenClaw (56,503)、Skills.sh (3,115)、Hermes-protocol 樣本 (100)。找到 751 個確認惡意程式；另有 ${STATS.ecosystem.maliciousFound.toLocaleString()} 個帶有至少一個可疑或已確認威脅訊號，而 Threat Cloud 累積黑名單為 ${STATS.ecosystem.blacklistedSkills.toLocaleString()} 個（三者為不同指標，不可互換）。249 個套件同時具備 shell、網路、檔案系統存取——「triple threat」組合，讓被攻陷的 skill 可以外送資料、安裝後門、保持持續性。122 個套件有 postinstall script，在任何審核之前就執行程式碼。`,

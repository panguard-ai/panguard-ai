...
export const blogPosts: BlogPost[] = [
  ...
  },
  {
    ...
    content: [
      ...
      'ATR ships as a single npm package with 650+ rules across 10 categories. Benchmarks: 99.7 percent precision and 63.2 percent recall on PINT (850 samples), ~98 percent recall on 650 NVIDIA Garak samples, and 100 percent recall with 97 percent precision and 0.20 percent false positives on the 498-sample real-world SKILL.md corpus. Sub-millisecond per-rule latency.',
  ...
  },
  {
    ...
    content: [
      ...
      'ATR ships 650+ rules across 10 categories. Real-world SKILL.md corpus: 100 percent recall, 97 percent precision, 0.20 percent false positives on 498 samples. PINT external corpus: 99.7 percent precision, 63.2 percent recall on 850 samples. Garak: ~98% recall on 650 samples. Detection runs sub-millisecond per scan.',
      ...
      'To run ATR as a runtime gate without writing your own loader, use [PanGuard Guard](https://github.com/panguard-ai/panguard-ai). It loads ATR at the agent boundary, enforces block decisions, and emits compliance evidence for EU AI Act, NIST AI RMF, and ISO/IEC 42001 audit. The community tier is free and unlimited.',
  ...
  },
  {
    ...
    content: [
      ...
      '4. Or run PanGuard Guard, which loads ATR rules at runtime and enforces them at the agent boundary. Free and open-source community tier. Enterprise tier adds compliance evidence and EU AI Act, NIST AI RMF, and ISO/IEC 42001 audit trails.',
  ...
  },
  {
    ...
    content: [
      ...
      '- **L2 Audit** — 🟢 Shipped. `pga scan` on MCP configs + SKILL.md, 650+ ATR rules, 98% recall on 650 samples for NVIDIA Garak, 0.48% FP on 3,115 real-world samples.',
      ...
      '## Numbers that matter',
      "The 650+ ATR detection rules are not ours. They are the community's. We maintain the standard, the pipeline, and the benchmarks:",
      '- **650+ rules**, MIT-licensed, in the [Agent Threat Rules](https://github.com/Agent-Threat-Rule/agent-threat-rules) repo',
      '- **98% recall** on 650 samples for NVIDIA Garak',
      '- **100% recall / 100% precision / 0% FP** on 341 real-world skills',
      ...
      '- **POC** — [panguard.ai/early-access](https://panguard.ai/early-access) to scope a proof-of-concept engagement.',
      '- **Enterprise** — [enterprise@panguard.ai](mailto:enterprise@panguard.ai) for on-prem, compliance reporting, AIAM, SLA.',
  ...
  },
  {
    ...
    content: [
      ...
      '- **L2 Audit** — 🟢 已出貨。`pga scan` 掃 MCP config + SKILL.md，650+ 條 ATR 規則，NVIDIA Garak 98% recall（650 樣本），3,115 個真實樣本 0.48% FP。',
      ...
      '## 有分量的數字',
      '650+ 條 ATR 偵測規則不是我們的，是社群的。我們維護的是標準、pipeline 與基準：',
      '- **650+ 條規則**，MIT 授權，放在 [Agent Threat Rules](https://github.com/Agent-Threat-Rule/agent-threat-rules) repo',
      '- **98% recall** 在 NVIDIA Garak (650 樣本)',
      '- **100% recall / 100% precision / 0% FP** 在 341 個真實 skill',
      ...
      '- **POC** — [panguard.ai/early-access](https://panguard.ai/early-access) 洽談概念驗證(proof-of-concept)合作。',
      '- **Enterprise** — 寫 [enterprise@panguard.ai](mailto:enterprise@panguard.ai) 談地端、合規報告、AIAM、SLA。',
  ...
  },
  {
    ...
    content: [
      ...
      '| PanGuard | Runs ATR rules + AI analysis on every skill | "a vehicle-API client skill: CRITICAL. Prompt injection found at line 47." |',
      ...
      'The attacks are happening now. An email-delivery MCP server silently forwarded inboxes to an attacker. A typosquatting cluster exfiltrated SSH keys from around 19 npm packages. 13.5% of ClawHub skills have security risk patterns.',
  ...
  },
  {
    ...
    content: [
      ...
      '這不是假設。一個電子郵件遞送 MCP server 前 15 個版本都很乾淨。某個小改版加了一行 BCC，每天靜默轉發每個組織數千封 email 到外部地址。另一個 typosquat 套件群植入了約 19 個 npm 仿冒套件（熱門 CLI 與 server 套件名稱的單字元變體），安裝的瞬間就偷走 SSH keys 和雲端憑證。範例已匿名，我們不點名套件。',
  ...
  },
  {
    ...
    content: [
      ...
      'We identified a cluster we track internally as SANDWORM_MODE: around 19 typosquatted packages targeting popular MCP tools, each a single-character variation of a widely used CLI or server package name. These packages had minimal functionality but contained postinstall scripts that harvested SSH keys and environment variables. All have been reported to npm; most were removed within hours. Package names are withheld here -- we do not publicly name packages as malicious without a reproducible public advisory.',
      '## What npm Should Do',
  ...
  },
  {
    ...
    content: [
      ...
      '## 仿冒攻擊正在進行中',
      '我們在內部追蹤一個叫 SANDWORM_MODE 的協同仿冒行動：約 19 個 typosquat 套件針對熱門 MCP 工具，每個都是熱門 CLI 或 server 套件名稱的單字元變體。這些套件功能極少但包含竊取 SSH keys 和環境變數的 postinstall scripts。全部已回報 npm，多數在數小時內被移除。此處不公開套件名稱 -- 我們不會在沒有可重現的公開 advisory 下公開指名套件為惡意。',
      '## npm 該怎麼做',
  ...
  },
  {
    ...
    excerpt:
      '一個電子郵件遞送 MCP server 發布了 15 個乾淨版本。某個小改版加了一行 BCC，每天將數千封郵件轉寄到攻擊者控制的信箱。另外還有一個 npm 仿冒套件群在竊取 SSH 金鑰。範例已匿名，我們不點名套件。',
    category: 'Threat Intelligence',
    date: '2026-03-23',
    author: 'Adam Lin',
    readingTime: '8 min',
    content: [
      ...
      '## 教訓',
      '對 MCP 的供應鏈攻擊不是理論。正在發生。攻擊模式跟公開記錄的 2018-2021 年 npm 事件一模一樣（event-stream、ua-parser-js、colors/faker），但影響更大，因為 MCP Skills 有更廣的系統存取權。\n\n防禦需要三件事：(1) 檢查工具行為而非只看依賴版本的預安裝掃描，(2) 標記版本間行為變更的 diff 分析，(3) 對照已知安全套件登錄的仿冒偵測。ATR 和 PanGuard 三項都有。',
    ],
  },
  {
  ...
  },
  {
    ...
    content: [
      ...
      'MCP08 -- No Audit Trail: Guard runs continuously as a system service. Logs every file access, process spawn, and skill installation. Daily summaries. Threat Cloud shares anonymous patterns for collective defense.',
      ...
      '## Try It',
      '```bash\nnpm install -g @panguard-ai/panguard && pga up\n```',
      "That's it. PanGuard detects your AI platforms, scans your installed skills, and starts protecting. `pga up` starts continuous monitoring with a dashboard.",
  ...
  },
  {
    ...
    content: [
      ...
      'MCP08 無稽核軌跡:Guard 以系統服務持續運行。記錄每個檔案存取、process 產生、skill 安裝。日摘要。Threat Cloud 分享匿名 pattern 做集體防禦。',
      ...
      '## 試試看',
      '```bash\nnpm install -g @panguard-ai/panguard && pga up\n```',
      '就這樣。PanGuard 偵測你的 AI 平台、掃你已裝的 skill、開始保護。`pga up` 啟動持續監控加儀表板。',
  ...
  },
  {
    ...
    content: [
      ...
      '- Read the spec: The full ATR specification is available at https://github.com/Agent-Threat-Rule/agent-threat-rules',
  ...
  },
  {
    ...
    content: [
      ...
      '- 讀規格:完整 ATR 規格在 https://github.com/Agent-Threat-Rule/agent-threat-rules',
  ...
  },
  {
    ...
    content: [
      ...
      'For teams that want continuous protection beyond the initial scan -- inline detection, automated response, and continuous monitoring -- Panguard Guard is included in the open-source package.',
      '## Get Started',
  ...
  },
  {
    ...
    content: [
      ...
      '想要超越初次掃描的持續保護 — inline 偵測、自動反應、持續監控 — 的團隊,Panguard Guard 包含在開源套件中。',
      '## 開始',
...

  {
    ...
    content: [
      ...
      '**Precision:** 100% on our benchmark corpus (341 real-world samples, 0 false positives)',
      '**Recall:** 100% on SKILL.md detection benchmark, 61.4% on the broader PINT benchmark (850 samples)',
  ...
  },
  {
    ...
    content: [
      ...
      '精準度 100%（341 個真實樣本，零誤報）。召回率 100%（SKILL.md benchmark）。平均延遲 3.36ms，53,577 個 skill 不到 5 分鐘掃完。',
...

  {
    ...
    content: [
      ...
      '- **An email-delivery MCP server** stole email inboxes by disguising itself as an email management tool',
      '- **A typosquatting cluster** exfiltrated SSH keys from around 19 npm packages',
  ...
  },
  {
    ...
    content: [
      ...
      '不是理論。已經有人被打了：',
      '- **一個電子郵件遞送 MCP server** 假裝郵件管理工具，實際上在偷整個信箱',
      '- **一個 typosquat 套件群** 埋在約 19 個仿冒 npm 套件裡，裝了就偷你 SSH key',
...

  {
    ...
    content: [
      ...
      '- **Zero false positives** -- 100% precision on our 341-sample benchmark.',
  ...
  },
  {
    ...
    content: [
      ...
      '## ATR 規則長這樣',
      '```yaml\nid: ATR-2026-00001\ntitle: System Prompt Override\nseverity: critical\ncategory: prompt-injection\ndetection:\n  patterns:\n    - regex: \"(?i)ignore\\\\s+(all\\\\s+)?previous\\\\s+instructions\"\n      target: input\nresponse: block\n```',
      'YAML 格式、regex 模式、零依賴。Python 能跑，Go 能跑，Java 能跑，什麼都能跑。平均 3.36ms 一次掃描，341 個真實樣本零誤報。',
      '## 相同的成長曲線',
...

  {
    ...
    content: [
      ...
      'As the EU AI Act phases in, organizations deploying AI agents will need to demonstrate they have assessed and mitigated risks. OWASP Agentic Top 10 is the leading framework for this assessment.',
      ...
      '- **SKILL.md Benchmark:** 100% recall, 100% precision, 0% FP on 341 real-world samples',
  ...
  },
  {
    ...
    content: [
      ...
      '## EU AI Act 逐步上路',
      '隨著 EU AI Act 逐步上路，用 AI agent 的組織要能證明自己評估過風險、有對應的防護措施。',
      ...
      '- SKILL.md Benchmark：100% 召回率，100% 精準度，0% 誤報（341 個真實樣本）',
  ...
  },
  {
    ...
    readingTime: '6 min',
    content: [
      '**ATR v2.1.1** is out on npm as `agent-threat-rules@2.1.1`. The headline: 314 rules at v2.0.17, **336 rules** at v2.1.1, a net add of 16 (or 17 if you count one minor reclassification). 98% recall on 650 samples for NVIDIA Garak `inthewild_jailbreak_llms` held.',
  ...
  },
  {
    ...
    excerpt:
      'ATR v2.1.1 新增 16 條規則(10 條自然語言攻擊 pattern + 6 條 skill 入侵延伸),以及一個讓全部既有規則都受惠的引擎升級。NVIDIA Garak 650 樣本 98% recall 維持不變。',
    category: 'Engineering',
    date: '2026-05-10',
    author: 'KUAN-HSIN LIN',
    readingTime: '6 min',
    content: [
      '**ATR v2.1.1** 上 npm 了,套件名 `agent-threat-rules@2.1.1`。標題:v2.0.17 有 314 條規則,v2.1.1 有 **336 條規則**,淨新增 16 條(如果把一條微小的重新分類算進去就是 17)。NVIDIA Garak `inthewild_jailbreak_llms` 上 650 樣本 98% recall 維持。',
      ...
      '每條規則都有明確的 true-positive 與 true-negative 範例,在 **432 條標註過的良性 skill corpus** 上跑 0 FP,並在 **650 條 NVIDIA Garak 樣本**上做 recall 衡量。',
      '## 6 條 Skill 入侵延伸',
  ...
  },
  {
    ...
    content: [
      ...
      '## Recall',
      'For recall we use NVIDIA Garak `inthewild_jailbreak_llms` (650 samples). ATR v2.1.1 lands at **98% recall** on 650 samples. We report recall and FP separately on different corpora because mixing them inflates both numbers.',
      ...
      '2. **Numbered results** — 96,096 / 751 / 432 / 98% are real cardinalities, not rounded marketing.',
  ...
  },
  {
    ...
    content: [
      ...
      '## Recall',
      'Recall 我們用 NVIDIA Garak `inthewild_jailbreak_llms`(650 個樣本)。ATR v2.1.1 是 **98% recall**(650 樣本)。recall 和 FP 我們分別在不同語料上報告,因為混在一起會同時膨脹兩個數字。',
      '## 為什麼這算 Audit-Grade',
      '三個性質:',
      '1. **語料來自公開 registry** — 任何人都能在容差範圍內重新抓取同一份 registry,重現輸入集。',
      '2. **數字都是真實計數** — 96,096 / 751 / 432 / 98% 是實際 cardinality,不是行銷四捨五入。',

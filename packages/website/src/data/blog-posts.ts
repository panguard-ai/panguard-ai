...
export const blogPosts: BlogPost[] = [
  {
    ...
    content: [
      ...
      '- `@mistralai/mistralai` — the official Mistral TypeScript SDK on npm, with the malicious payload delivered via the `preinstall` lifecycle hook invoking `node setup.mjs`.\n- `mistralai` and `guardrails-ai` — the official Mistral and Guardrails AI Python SDKs on PyPI, with the payload triggered by an `on import` side effect in `__init__.py` that downloaded `/tmp/transformers.pyz` from `git-tanstack[.]com`.\n- TanStack packages (`@tanstack/router`, `@tanstack/query`, others) — same family, npm `prepare` lifecycle hook executing through a Bun runtime drop.',
      'The Cyera and Wiz analyses count 404 trojanized package versions in total, against a cumulative 518 million downloads of the affected packages over the prior twelve months.',
      '## What the payload does',
      'Three artifacts characterize the wave:',
      "1. **`gh-token-monitor`** — a persistence daemon installed via macOS `LaunchAgent` (`com.github.token.monitor.plist`) or Linux `systemd` service (`gh-token-monitor.service`). The daemon polls `api.github.com/user` every sixty seconds using the developer's GitHub token.",
      '2. **Dead-man\'s switch** — when the polled token returns HTTP 401 (the developer rotated or revoked it), the daemon executes a destructive cleanup. Observed variants run `rm -rf ~/.config`, `rm -rf ~/.aws`, `rm -rf ~/.ssh`, or `shutil.rmtree` over the equivalent paths. The destructive action is conditional, which is what makes the daemon hard to catch with static manifest scanning alone.',
      ...
      'Mistral spun up a public security advisory page at `docs.mistral.ai/resources/security-advisories` on 2026-05-13, acknowledging the TypeScript SDK compromise. That is the first time Mistral has run a public security advisory page. It is also the right action.',
  ...
  },
  {
    ...
    content: [
      ...
      '- `@mistralai/mistralai` — Mistral 官方 TypeScript SDK on npm，惡意 payload 透過 `preinstall` lifecycle hook 呼叫 `node setup.mjs` 投放。\n- `mistralai` 和 `guardrails-ai` — Mistral 和 Guardrails AI 官方 Python SDK on PyPI，payload 由 `__init__.py` 的 `on import` side effect 觸發，下載 `/tmp/transformers.pyz` 從 `git-tanstack[.]com`。\n- TanStack packages（`@tanstack/router`、`@tanstack/query` 等）— 同一家族，npm `prepare` lifecycle hook 透過 Bun runtime 投放。',
      'Cyera 和 Wiz 的分析估計總共 404 個 trojanized package versions，累積過去十二個月 5 億 1,800 萬次下載。',
      '## Payload 做什麼',
      '這波攻擊有三個 artifact:',
      "1. **`gh-token-monitor`** — 持續駐留的 daemon，透過 macOS `LaunchAgent`（`com.github.token.monitor.plist`）或 Linux `systemd` service（`gh-token-monitor.service`）安裝。Daemon 用開發者的 GitHub token 每 60 秒 poll 一次 `api.github.com/user`。",
      '2. **Dead-man\'s switch** — 當 polled token 回 HTTP 401（開發者輪換或撤銷 token），daemon 觸發毀滅性清理。觀察到的變體會跑 `rm -rf ~/.config`、`rm -rf ~/.aws`、`rm -rf ~/.ssh`，或 Python 端用 `shutil.rmtree` 對應路徑。毀滅動作是條件觸發的，這就是為什麼純靜態 manifest 掃描抓不到 daemon。',
      ...
      'Mistral 在 2026-05-13 開了一個公開 security advisory 頁面 `docs.mistral.ai/resources/security-advisories`，承認 TypeScript SDK 中標。這是 Mistral 第一次跑公開 security advisory 頁面。也是對的動作。',
  ...
  },
  {
    ...
    content: [
      ...
      '23,000 npm downloads in the trailing thirty days as of 2026-05-12.',
      '## Honest scope',

...
export const blogPosts: BlogPost[] = [
  {
    ...
    content: [
      ...
      'SAFE-MCP will define what needs to be secured. ATR provides 108 executable rules that detect these attacks in real time. They solve different problems at different layers:',
      ...
      '- 108 ATR detection rules covering OWASP Agentic Top 10: 10/10 categories',
      ...
      'Free. Open source. 108 detection rules. 16 platforms. The standard is coming. The protection is already here.',
    ],
  },
  {
    ...
    content: [
      ...
      'ATR（Agent Threat Rules）提供怎麼偵測這些威脅。108 條 YAML 格式的偵測規則，每條都有 regex 模式、嚴重性分類、OWASP 映射和建議的回應行動。',
      ...
      '掃描你的 Skills。只要 60 秒。',
      '```bash\ncurl -fsSL https://get.panguard.ai | bash\n```',
      '免費。開源。108 條偵測規則。16 個平台。標準還在路上。防護已經在了。',
      'https://panguard.ai',
    ],
  },
  {
    ...
    content: [
      '## The Numbers',
      'We crawled every publicly listed skill on ClawHub -- 36,394 packages total. Of those, 9,676 had parseable source content (README, SKILL.md, or tool definitions). We ran all 108 ATR detection rules against every one of them. The scan took 14 hours on a single machine.',
      ...
      'Install PanGuard and scan your local skills:\n\n```bash\nnpm install -g @panguard-ai/panguard && pga up\n```\n\nThis runs all 108 ATR rules against your installed skills and gives you a per-skill risk report. It takes about 60 seconds. The scan results feed into Threat Cloud, so every scan makes the detection network stronger for everyone.',
  ...
  },
  {
    ...
    content: [
      '## 數字說話',
      '我們爬取了 ClawHub 上所有公開 Skills -- 共 36,394 個套件。其中 9,676 個有可解析的原始內容（README、SKILL.md 或工具定義）。我們用全部 108 條 ATR 偵測規則掃描每一個。單機跑了 14 小時。',
      ...
      '## 你現在就能做的事',
      '安裝 PanGuard 掃描本地 Skills：\n\n```bash\nnpm install -g @panguard-ai/panguard && pga up\n```\n\n這會用全部 108 條 ATR 規則掃描已安裝的 Skills，產出逐一的風險報告。大約 60 秒。掃描結果會回饋 Threat Cloud，每次掃描都讓整個偵測網路更強。',
  ...
  },
  {
    ...
    content: [
      '## Methodology',
      'We pulled every npm package matching the queries `mcp-server`, `mcp-tool`, `model-context-protocol`, and `@modelcontextprotocol` from the npm registry. This yielded 2,386 unique packages containing a combined 35,858 tool definitions. We ran ATR v0.4.0 (108 rules) against every package. We also inspected package.json for postinstall scripts, analyzed dependency trees for known vulnerabilities, and checked for typosquatting against the top 100 most-downloaded MCP packages.',
      '## Headline Numbers',
  ...
  },
  {
    ...
    content: [
      '## 方法論',
      '我們從 npm registry 拉取所有符合 `mcp-server`、`mcp-tool`、`model-context-protocol`、`@modelcontextprotocol` 查詢的套件。共 2,386 個不重複套件，包含 35,858 個工具定義。我們用 ATR v0.4.0（108 條規則）掃描每一個，同時檢查 package.json 的 postinstall scripts、分析依賴樹的已知弱點、對照前 100 名最多下載的 MCP 套件做仿冒偵測。',
      '## 核心數據',
  ...
  },
  {
    ...
    excerpt:
      'ATR (Agent Threat Rules) is an open detection standard for AI agent security threats. 108 YAML rules. OWASP Agentic Top 10 full coverage. Think Snort for network, Sigma for endpoints, ATR for AI agents.',
  ...
  },
  {
    ...
    excerpt:
      'ATR（Agent Threat Rules）是針對 AI Agent 安全威脅的開放偵測標準。108 條 YAML 規則。OWASP Agentic Top 10 全覆蓋。網路有 Snort，端點有 Sigma，AI Agent 有 ATR。',
    category: 'Product Updates',
    date: '2026-03-21',
    author: 'Panguard AI Team',
    readingTime: '8 min',
  ...
  },
  {
    ...
    title: 'OWASP Agentic Top 10: How 108 ATR Rules Cover All 10 Categories',
    excerpt:
  ...
  },
  {
    ...
    title: 'OWASP Agentic Top 10：108 條 ATR 規則如何覆蓋全部 10 個類別',
    excerpt:
  ...
  },
  {
    ...
    content: [
      ...
      'A badge that says "Certified Secure" implies completeness. It suggests that everything dangerous has been found and eliminated. At 62.7% recall, we cannot make that claim. "ATR Scanned" means: this package has been analyzed against 108 known attack patterns and no threats were detected. It does not mean the package is safe. It means it passed the checks we have.',
  ...
  },
  {
    ...
    content: [
      ...
      '寫著「Certified Secure」的徽章暗示完整性。它暗示所有危險都已被發現和消除。在 62.7% 召回率下，我們無法做這個聲明。「ATR Scanned」的意思是：這個套件已被 108 條已知攻擊模式分析過，未偵測到威脅。不代表套件是安全的，代表它通過了我們有的檢查。如果開發者看到「Certified」就不再審查便安裝，而套件包含一個 ATR 漏掉的改寫式注入，我們反而讓開發者更不安全——給了他們虛假的信心。',
      '## Snort 的先例',
  ...
  },
  {
    ...
    content: [
      ...
      'MCP03 -- Tool Poisoning: Our core strength. 13 prompt injection patterns + 108 ATR detection rules + context signal engine. We catch hidden instructions in tool descriptions, `<IMPORTANT>` block stealth attacks, and silent data exfiltration commands.',
  ...
  },
  {
    ...
    content: [
      ...
      'The agent runs 108 ATR (Agent Threat Rules) detection rules purpose-built for AI agent security. It monitors tool calls, prompt flows, skill behavior, and agent interactions. When it detects a threat, it responds automatically -- revoking skills, blocking malicious actions, quarantining compromised agents -- and explains what happened in plain language through Slack, Telegram, or email.',
  ...
  },
  {
    ...
    content: [
      ...
      'Cost. Running 108 ATR rules requires minimal compute. Running an LLM for every security event would cost orders of magnitude more. The rules engine adds less than 2% CPU overhead. An AI-only approach would be economically impossible at this scale.',
  ...
  },
  {
    ...
    content: [
      ...
      '```\ncurl -fsSL https://get.panguard.ai | bash\n```',
      'The scan engine checks 108 ATR detection rules against your AI agent environment. It examines skill configurations, tool permissions, prompt flows, and dozens of other AI agent attack surface vectors.',
  ...
  },
  {
    ...
    content: [
      ...
      'This includes the 108 ATR detection rules that power our rules engine. It includes the scanning logic for skill configurations, tool permissions, prompt flows, and every other check the engine performs.',
      '## Why MIT License',
  ...
  },
  {
    ...
    content: [
      ...
      '## 108 Rules Covering 9 Attack Categories',
      'ATR ships with 108 detection rules across 9 categories:',
...

  {
    ...
    content: [
      ...
      '108 rules. 9 threat categories. Sub-5ms latency. MIT licensed. The ecosystem needs a shared defense, and the scan data shows exactly why.',
      '---',
  ...
  },
  {
    ...
    title: '60 天 30 個 CVE：MCP 攻擊面正在爆炸式成長',
    excerpt:
      'Model Context Protocol 在兩個月內從零增長到 30 個 CVE。38% 的 MCP 伺服器沒有認證。這是數據。',
    category: 'Threat Intelligence',
    date: '2026-04-09',
    author: 'ATR Research',
    readingTime: '8 min',
    content: [
      '## 60 天內從零到 30 個 CVE',
      'Model Context Protocol 是 AI 領域成長最快的攻擊面。在 2026 年 2 月到 4 月的 60 天內，針對 MCP 伺服器和工具提交了 30 個 CVE。平均每兩天一個新漏洞。',
      '做個對比：Docker 前兩年有 7 個 CVE。Kubernetes 第一年有 12 個。MCP 兩個月就有 30 個。',
      '## 數據',
      '| 指標 | 數值 |',
      '|------|------|',
      '| CVE（60 天） | 30 |',
      '| 零認證的 MCP 伺服器 | 38% |',
      '| Registry 中的 MCP skills | 53,577+ |',
      '| 有安全問題的 skills | 946 (1.77%) |',
      '| 嚴重等級 | 875 |',
      '## MCP 為什麼不同',
      '傳統 API 有明確的信任邊界。你呼叫一個 API，它回傳資料。API 不能讀取你的檔案、在你的機器上執行命令、或存取你的憑證。',
      'MCP 工具沒有這種邊界。當 AI agent 呼叫一個 MCP skill，該 skill 以 agent 的完整權限運行。檔案系統存取。Shell 執行。網路請求。憑證儲存。全部。',
      '這不是理論上的擔憂。已有真實攻擊被記錄：',
      '- **postmark-mcp** 偽裝成郵件管理工具竊取信箱',
      '- **SANDWORM_MODE** 從 19 個 typosquat 套件中竊取 SSH 金鑰',
      '- 在 OpenClaw registry 中，674 個 skills 發現了工具描述下毒',
      '## 認證缺口',
      '38% 的 MCP 伺服器零認證。沒有 API key。沒有 OAuth。沒有任何形式的驗證。攻擊者可以建立惡意 MCP 伺服器，在公開 registry 註冊，然後等 AI agent 連接。',
      'MCP 規範本身不強制要求認證。它是可選的。大多數實作都跳過了。',
      '## ATR 偵測什麼',
      'ATR 目前以 108 條偵測規則覆蓋以下 MCP 攻擊向量：',
      '- **提示注入**（33 條）— 透過精心構造的輸入劫持 agent 行為',
      '- **Skill 入侵**（22 條）— 惡意或有漏洞的 MCP skills 和 SKILL.md',
      '- **上下文竊取**（14 條）— 竊取對話上下文和敏感資料',
      '- **工具下毒**（22 條）— 被下毒的工具描述和惡意回應',
      '- **權限提升**（8 條）— 未授權提升 agent 能力',
      '標準覆蓋：',
      '- OWASP Agentic Top 10：**10/10**',
      '- SAFE-MCP (OpenSSF)：**78/85 技術 (91.8%)**',
      '## 速度問題',
      '委員會式安全標準需要 12-18 個月才能發布。SAFE-MCP 已獲資助但尚未發布。OWASP 框架按季度更新。',
      '新的 MCP CVE 每 2 天出現一個。',
      'ATR 用社群驅動規則和自動威脅結晶來縮小這個差距。發現新攻擊模式後，Threat Cloud 分析它、產生偵測規則、社群審查後合併。從發現到防護的平均時間：不到 1 小時。',
      '## 你的組織該怎麼做',
      '如果你以任何方式使用帶有 MCP 工具的 AI agent — 開發、客服、資料分析、自動化 — 你就暴露在這個攻擊面中。',
      '今天就做三件事：',
      '1. **盤點你的 MCP skills。** 知道你的 agent 有權存取什麼。',
      '2. **用 ATR 掃描。** `npx agent-threat-rules scan` 只需幾秒。',
      '3. **關注 CVE 動態。** 新漏洞每兩週出現。',
      'MCP 攻擊面的增長速度超過了近年來任何協議。60 天 30 個 CVE 只是開始。',
      '---',
      '*ATR 提供這些威脅的開源偵測規則：[agentthreatrule.org](https://agentthreatrule.org)*',
    ],
  },
  ...
  },
  {
    ...
    title: 'ATR vs YARA vs Sigma：AI Agent 的偵測規則標準',
    excerpt:
      'YARA 偵測惡意軟體。Sigma 偵測入侵。ATR 偵測 AI agent 威脅。偵測標準如何隨新攻擊面演化。',
    category: 'Engineering',
    date: '2026-04-09',
    author: 'ATR Research',
    readingTime: '10 min',
    content: [
      '## 每個新攻擊面都會誕生自己的偵測標準',
      '當惡意軟體成為大規模問題時，業界建立了 YARA。當 SIEM 日誌分析需要可攜帶的規則時，業界建立了 Sigma。現在 AI agent 是成長最快的攻擊面，業界需要專屬的偵測標準。',
      'ATR（Agent Threat Rules）就是這個標準。以下是比較。',
      '## 對比',
      '| | YARA | Sigma | ATR |',
      '|---|------|-------|-----|',
      '| **用途** | 惡意軟體偵測 | 日誌型入侵偵測 | AI agent 威脅偵測 |',
      '| **輸入** | 二進制檔案 | 日誌事件 (JSON/text) | 工具描述、提示、skill metadata |',
      '| **規則格式** | 自定義 DSL | YAML | YAML |',
      '| **執行** | 位元組模式匹配 | 轉換為 SIEM 查詢 | 文字 regex 匹配 |',
      '| **延遲** | 每檔案 ~100ms | 取決於 SIEM | 每次掃描 < 5ms |',
      '| **規則數** | 10,000+（社群） | 3,000+（SigmaHQ） | 108（持續成長） |',
      '| **主要採用者** | VirusTotal, CrowdStrike | Splunk, Elastic, Sentinel | Cisco AI Defense |',
      '| **授權** | BSD | LGPL | MIT |',
      '## 為什麼不直接用 YARA 或 Sigma？',
      '因為輸入根本不同。',
      '**YARA** 操作二進制檔案。它找位元組序列、十六進制模式和檔案結構。AI agent 威脅不是二進制 — 它們是嵌入在工具描述、提示和 skill metadata 中的自然語言指令。',
      '**Sigma** 操作結構化日誌事件。AI agent 威脅不存在於日誌中 — 它們存在於 agent 處理的內容裡，發生在任何日誌記錄之前。等到 Sigma 規則能偵測到攻擊時，agent 已經執行了。',
      '**ATR** 操作 AI agent 消費的文字：工具描述、SKILL.md 檔案、使用者提示和 agent 回應。偵測發生在 agent 行動之前，不是之後。',
      '## ATR 規則如何運作',
      '```yaml\nid: ATR-2026-00001\ntitle: System Prompt Override\nseverity: critical\ncategory: prompt-injection\ndetection:\n  patterns:\n    - regex: \"(?i)ignore\\\\s+(all\\\\s+)?previous\\\\s+instructions\"\n      target: input\n  condition: any\nresponse: block\n```',
      '設計原則：',
      '- **亞毫秒執行** — regex 很快。平均掃描 3.36ms。',
      '- **零依賴** — 純 regex，任何語言都能執行。',
      '- **零誤報** — 498 樣本基準測試上 100% 精準度。',
      '- **可組合** — YAML 檔案。用 Python、Go、Java、Rust 隨你解析。',
      '## 成長軌跡',
      'YARA 從 VirusTotal 一個研究員的幾條規則開始。今天有 10,000+ 條社群規則，是全球惡意軟體偵測標準。',
      'Sigma 從 Florian Roth 和幾百條規則開始。今天有 3,000+ 條規則，8,000+ GitHub stars，整合在每個主要 SIEM 中。',
      '兩者都遵循相同的模式：**標準格式 + 開放規則 + 一次重大採用** 觸發指數級成長。',
      'ATR 正在這條曲線的起點：',
      '- 108 條規則跨 9 個威脅類別',
      '- Cisco AI Defense 採用 34 條 ATR 規則作為上游',
      '- OWASP Agentic Top 10：10/10 覆蓋',
      '- 53,577 個 skills 掃描 — 史上最大規模 AI agent 安全審計',
      '## 什麼時候用什麼',
      '| 威脅類型 | 使用 |',
      '|---------|------|',
      '| 下載檔案中的惡意軟體 | YARA |',
      ...
      '| Agent 輸入中的提示注入 | ATR |',
      '| Agent 回應中的工具下毒 | ATR |',
      '它們是互補的，不是競爭的。完整的安全堆疊三者都用。',
      '```bash\nnpm install agent-threat-rules\n```',
      '---',
      '*ATR 是 MIT 授權的社群驅動專案。規則、引擎和基準測試全部開源。*',
    ],
  },
  ...
  },
  {
    ...
    title: 'Cisco 如何在 AI Defense 中使用 ATR 規則',
    excerpt:
      'Cisco AI Defense 合併了 34 條 ATR 規則作為上游依賴。然後他們建了 CLI 來消費 ATR rule packs。整合方式解析。',
    category: 'Industry',
    date: '2026-04-09',
    author: 'ATR Research',
    readingTime: '6 min',
    content: [
      '## 從 PR 到生產環境，3 天',
      '2026 年 3 月，Cisco AI Defense 的一位工程師向 ATR repo 提交了 pull request。PR #79：1,272 行新增。34 條 ATR 偵測規則移植到他們的 skill-scanner 程式碼庫中。',
      '我們審查了。3 天合併。',
      '然後有趣的事發生了：Cisco 專門建了一個 `--rule-packs` CLI 旗標來消費 ATR 規則。他們不只是複製規則 — 他們建了基礎設施來和 ATR 保持同步。',
      '這就是企業採用偵測標準的樣子。',
      '## 他們整合了什麼',
      '34 條規則涵蓋：',
      '- 提示注入模式（系統提示覆蓋、指令劫持）',
      '- 工具下毒（工具描述中的隱藏指令）',
      '- 上下文竊取（透過 agent 回應洩漏資料）',
      '- 憑證收割（環境變數存取、金鑰檔案讀取）',
      '這些規則在 Cisco AI Defense 的生產 skill scanner 中運行。當 skill 被提交到他們的平台時，在被批准前會先用 ATR 規則掃描。',
      '## 整合模式',
      'Cisco 的整合展示了任何平台都可以複製的模式：',
      '```bash\n# 1. 安裝 ATR 規則\nnpm install agent-threat-rules\n\n# 2. 規則是 node_modules/agent-threat-rules/rules/ 中的 YAML 檔案\n# 3. 用你選擇的語言解析 YAML\n# 4. 對輸入執行 regex 模式匹配\n```',
      'ATR 規則刻意保持簡單。YAML 格式。Regex 模式。無執行時依賴。任何語言都能消費。',
      '## 為什麼這很重要',
      '當財富 500 強安全廠商專門建構工具來消費你的規則格式時，它以 benchmark 無法做到的方式驗證了標準。',
      '這意味著：',
      '1. **格式已經可用於生產環境。** Cisco 安全工程師審查了 34 條規則，認為可靠到足以用於他們的生產 scanner。',
      '2. **規則能捕捉真實威脅。** 不是學術演練。它們偵測 Cisco 客戶面臨的攻擊。',
      '3. **模式可複製。** 任何平台都能走同樣的整合路徑。安裝、解析、執行。',
      '## 上游模式',
      'ATR 的運作方式像 Linux 發行版的套件庫。ATR 發布偵測內容。消費者（Cisco，以及即將到來的其他人）將規則作為上游依賴拉取。',
      '當 ATR 新增規則，消費者更新依賴版本即可。當消費者發現誤報或繞過方法，他們回報並改善共享規則集。',
      '這就是網絡效應：每個消費者都讓規則對其他所有消費者變得更好。',
      '## 你的平台也能這樣做',
      '整合指南：[agentthreatrule.org/en/integrate](https://agentthreatrule.org/en/integrate)',
      'GitHub 上的 PR #79：[cisco-ai-defense/skill-scanner#79](https://github.com/cisco-ai-defense/skill-scanner/pull/79)',
      '```bash\nnpm install agent-threat-rules\n```',
      '---',
      '*ATR 是 AI agent 安全的開放偵測標準。108 條規則。MIT 授權。*',
    ],
  },
  ...
  },
  {
    ...
    title: 'OWASP Agentic Top 10：完整偵測覆蓋對應',
    excerpt:
      'ATR 以可執行偵測規則覆蓋 10/10 OWASP Agentic Top 10 類別。完整對應表。',
    category: 'Compliance',
    date: '2026-04-09',
    author: 'ATR Research',
    readingTime: '8 min',
    content: [
      '## 10/10 覆蓋。可執行規則，不是清單。',
      'OWASP Agentic Top 10 定義了 AI agent 最關鍵的安全風險。它是一份清單。ATR 提供讓清單可執行的偵測規則。',
      '## 覆蓋對應',
      '| OWASP ID | 威脅 | ATR 規則 | 嚴重度 | 狀態 |',
      '|----------|------|---------|--------|------|',
      '| ASI01 | Agent 目標劫持 | ATR-PI-001 到 ATR-PI-033 | 嚴重 | 已覆蓋 |',
      '| ASI02 | 工具誤用 | ATR-TP-001 到 ATR-TP-022 | 嚴重-高 | 已覆蓋 |',
      '| ASI03 | 權限提升 | ATR-PE-001 到 ATR-PE-008 | 高 | 已覆蓋 |',
      '| ASI04 | 知識污染 | ATR-DP-001 | 中 | 已覆蓋 |',
      '| ASI05 | 記憶操控 | ATR-AM-001 到 ATR-AM-005 | 高 | 已覆蓋 |',
      '| ASI06 | 過度自主 | ATR-EA-001 到 ATR-EA-002 | 中 | 已覆蓋 |',
      '| ASI07 | 級聯幻覺 | ATR-MS-001 | 中 | 已覆蓋 |',
      '| ASI08 | 不受控鏈接 | ATR-EA-002, ATR-AM-003 | 高 | 已覆蓋 |',
      '| ASI09 | 信任邊界違反 | ATR-CE-001 到 ATR-CE-014 | 嚴重 | 已覆蓋 |',
      '| ASI10 | 供應鏈入侵 | ATR-SC-001 到 ATR-SC-022 | 嚴重 | 已覆蓋 |',
      '## 為什麼這對合規很重要',
      'EU AI Act 在 2026 年 8 月生效。部署 AI agent 的組織需要證明他們已經評估和減緩了風險。OWASP Agentic Top 10 是這項評估的主要框架。',
      '但沒有偵測的框架只是一份 PDF。ATR 提供：',
      '- **自動掃描** 覆蓋全部 10 個類別',
      '- **合規證據** 用於審計（帶時間戳、規則 ID 和判定的掃描報告）',
      '- **持續監控** 透過 CI/CD 整合或執行時掃描',
      '## 超越 OWASP',
      'ATR 還覆蓋：',
      '- **SAFE-MCP (OpenSSF):** 78/85 技術 = 91.8% 覆蓋',
      '- **OWASP Skills Top 10:** 7/10',
      '- **PINT Benchmark:** 99.7% 精準度，61.4% 召回率',
      '- **SKILL.md Benchmark:** 96.9% 召回率，100% 精準度，0% 誤報',
      '完整覆蓋對應：[agentthreatrule.org/zh/coverage](https://agentthreatrule.org/zh/coverage)',
      '---',
      '*ATR 是 AI agent 安全的開放偵測標準。MIT 授權。社群驅動。*',
    ],
  },
];

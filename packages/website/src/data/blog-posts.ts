...
export const blogPosts: BlogPost[] = [
  {
    ...
    title: "PanGuard vs OWASP MCP Top 10: What We Cover, What We Don't, and What's Next",
    excerpt:
      "OWASP published the first official threat taxonomy for AI agents. We mapped every category against PanGuard's detection capabilities -- honestly, including the gaps.",
    category: 'Threat Intelligence',
    date: '2026-03-25',
    author: 'Panguard AI Team',
    readingTime: '8 min',
    content: [
      '## The MCP Security Problem Now Has a Name',
      'On March 15, 2026, OWASP published the MCP Top 10 -- the first official security standard for AI agent tool integrations. Led by Vandana Verma Sehgal, the project catalogs the ten most critical threats facing MCP (Model Context Protocol) deployments.',
      "This matters because MCP is how AI agents like Claude Code, Cursor, and Codex connect to external tools. Every MCP skill runs with the agent's permissions -- your files, your credentials, your shell. Until now, there was no standard way to talk about what can go wrong.",
      'Now there is. And it validates the exact problem we built PanGuard to solve.',
      '## The Honest Scorecard',
      "We mapped every OWASP MCP category against PanGuard's actual detection capabilities. Not marketing capabilities. Actual, tested, verified capabilities.",
      '### What We Cover (7 out of 10)',
      '**MCP01: Token Mismanagement and Secret Exposure**',
      'PanGuard detects hardcoded secrets using 40+ patterns: AWS access keys, GitHub tokens, private keys, database connection strings, and API keys across all major providers. Guard monitors `.env`, `.ssh/`, and `.aws/` directories in real-time and alerts if anything tries to read them.',
      'Coverage: strong. We catch secrets at rest (in skill files) and in motion (runtime file access).',
      '**MCP03: Tool Poisoning**',
      'This is our core strength. The Skill Auditor runs 13 prompt injection patterns, 61 ATR (Agent Threat Rules) detection rules, and a context signal engine that distinguishes legitimate tools from malicious ones.',
      "The OWASP report highlights three tool poisoning variants: hidden instructions in tool descriptions, tool shadowing that redirects functionality, and rug pulls that modify definitions after approval. PanGuard's pre-install audit catches the first two. The third (runtime modification) is caught by Guard's file monitoring and skill hash tracking.",
      'We recently added detection for `<IMPORTANT>` block stealth attacks and silent data exfiltration instructions -- patterns that bypass traditional regex by hiding malicious intent in markup structures.',
      'Coverage: strong. This is what PanGuard was built for.',
      '**MCP04: Supply Chain Attacks and Dependency Tampering**',
      'The Skill Auditor checks dependency declarations for known vulnerabilities, typosquatting (similar package names), and suspicious install scripts. Every skill gets a dependency analysis before installation.',
      "The OWASP report mentions registry poisoning -- a compromised database connector that logged query parameters to external endpoints. PanGuard's ATR rules include patterns for exactly this: data exfiltration via `curl`, `fetch`, and network calls combined with credential file access.",
      'Coverage: moderate. We catch declared dependencies and known patterns. Sophisticated supply chain attacks that only manifest at runtime are harder to catch statically.',
      '**MCP05: Command Injection and Execution**',
      'PanGuard detects reverse shells (`bash -i >& /dev/tcp/`), privilege escalation (`sudo`, `chmod 777`), remote code execution (`curl | bash`), and environment variable exfiltration (`printenv | curl`). Our two-pass matching system checks both the raw content and prose-only content (code blocks stripped) to reduce false positives while maintaining detection accuracy.',
      'The OWASP report cites "Clinejection" -- malicious GitHub issue titles triggering `npm install` on poisoned packages. PanGuard\'s tool poisoning patterns catch the command injection component, though the social engineering vector (crafting issue titles) is outside our scope.',
      'Coverage: strong for the command patterns. Social engineering delivery vectors are outside scope.',
      '**MCP06: Intent Flow Subversion**',
      "This is prompt injection by another name -- malicious instructions embedded in retrieved context that hijack agent decision-making. PanGuard's 13 injection patterns cover identity override, instruction hijacking, jailbreak attempts, system prompt manipulation, and our newest additions: `<IMPORTANT>` block hidden instructions and silent exfiltration commands.",
      'Our context signal engine also detects description-behavior mismatches: a skill that claims to be a "simple calculator" but contains `rm -rf` or credential access commands gets flagged with a boosted risk multiplier.',
      'Coverage: strong. Pre-install detection catches these before the skill ever runs.',
      '**MCP08: Lack of Audit and Telemetry**',
      'Guard runs 24/7 as a system service, logging every file access, process spawn, network call, and skill installation. Events are stored locally with daily summaries. The Threat Cloud flywheel anonymously shares threat patterns (not data) to strengthen collective defense.',
      'Coverage: strong. Guard was designed specifically to fill this gap.',
      '**MCP10: Context Injection and Over-Sharing**',
      'PanGuard detects context exfiltration patterns: skills that try to read conversation history, access cross-session data, or expose credentials through tool invocations. The ATR rules include patterns for credential harvesting via environment variables and file system access.',
      'Coverage: partial. We catch exfiltration attempts at the skill level. Cross-session context leaks in the LLM runtime itself are outside our detection scope -- that requires changes in the model hosting layer.',
      "## What We Don't Cover (3 out of 10)",
      'Honesty matters more than marketing. Here are the gaps.',
      '**MCP02: Privilege Escalation via Scope Creep**',
      "PanGuard audits permissions at install time. We check what a skill declares it needs and flag over-permissioned skills. But we don't track permission drift over time -- a skill that starts with read access and gradually acquires write and execute permissions would not be caught by our current architecture.",
      'This is a runtime monitoring problem. Guard watches file access and process spawning, which partially mitigates this, but we don\'t have explicit "permission budget" tracking. It\'s on our roadmap.',
      '**MCP07: Insufficient Authentication and Authorization**',
      'The OWASP report found that 38% of 500+ scanned MCP servers lack authentication entirely. PanGuard does not currently inspect MCP server configurations for authentication strength.',
      'We detect what skills do, not how servers are configured. Adding MCP server auth scanning to `pga setup` is straightforward and planned.',
      '**MCP09: Shadow MCP Servers**',
      "PanGuard's setup detects 8 known AI platforms (Claude Code, Claude Desktop, Cursor, OpenClaw, Codex, WorkBuddy, NemoClaw, ArkClaw). But unauthorized MCP deployments running on non-standard ports or paths would be missed.",
      "Full shadow MCP discovery would require network scanning and process enumeration beyond our current scope. Guard's process monitoring partially covers this -- it would detect suspicious processes spawning -- but explicit shadow MCP hunting is not implemented.",
      '## Why the Gaps Matter Less Than You Think',
      "The three gaps share a pattern: they're all runtime concerns. PanGuard is primarily a pre-deployment tool. We audit before you install, not after.",
      "This is by design. The highest-value intervention point is before a malicious skill touches your system. Once it's running, you're playing defense. We'd rather you never install it in the first place.",
      "Guard adds a runtime layer, but it's a safety net, not the primary defense. The audit is the primary defense.",
      '## What This Means for the Industry',
      'The OWASP MCP Top 10 does something critical: it gives security teams a shared vocabulary. Before this, conversations about AI agent security were ad-hoc. "We need to secure our MCP deployment" could mean anything. Now it means addressing MCP01 through MCP10.',
      "For PanGuard, it validates our approach. We didn't build against the OWASP list -- it didn't exist when we started. We built against the threats we observed in the wild. The fact that 7 of 10 categories align with our existing detection rules suggests we were looking at the right problems.",
      'For the industry, it means the "nobody is doing this" era is over. AI agent security is now a named discipline with official taxonomy. Expect compliance requirements, audit checklists, and procurement criteria to reference OWASP MCP within 12 months.',
      '## Try It',
      'PanGuard is 100% open source, MIT licensed, and free.',
      '```bash\nnpm install -g @panguard-ai/panguard\npga setup\n```',
      'One command to install. One command to protect every AI agent on your machine.',
      '`pga audit skill ./path` to audit any skill before installing it.',
      '`pga up` to start 24/7 protection with a browser dashboard.',
      "The OWASP MCP Top 10 tells you what can go wrong. PanGuard helps you make sure it doesn't.",
      '---',
      '*The OWASP MCP Top 10 is available at [mcpblog.dev](https://mcpblog.dev/blog/2026-03-15-owasp-mcp-top-10). PanGuard is open source at [github.com/panguard-ai/panguard-ai](https://github.com/panguard-ai/panguard-ai).*',
    ],
  },
  {

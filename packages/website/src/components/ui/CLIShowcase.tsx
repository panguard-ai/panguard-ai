...
const tabContent: Record<TabId, Line[]> = {
  ...
  guard: [
    ...
    { text: '' },
    { text: `  Rule Engine (ATR)        LOADED  ${STATS.totalRulesDisplay} rules`, color: c.ok },
    { text: '  Behavioral Baseline      READY   on-device', color: c.ok },
    { text: '  Correlation & AST        READY   on-device', color: c.ok },
        ...
        {tabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={`relative px-3 sm:px-5 py-2.5 text-xs font-mono font-medium transition-colors shrink-0 ${
                isActive

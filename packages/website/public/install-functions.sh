#!/usr/bin/env bash
# Panguard AI Installer Functions
# Sourced by install.sh and by tests. Does NOT execute when sourced.
#
# Usage in tests:
...
# ── Colors (set only if not already set, so tests can override) ──
: "${RED:='\033[0;31m'}"
: "${GREEN:='\033[0;32m'}"
: "${YELLOW:='\033[1;33m'}"
: "${BLUE:='\033[0;34m'}"

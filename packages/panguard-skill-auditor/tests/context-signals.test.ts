...
describe('detectContextSignals', () => {
  ...
  it('reduces severity for declared Bash in allowed-tools', () => {
    const manifest = { name: 'dev-tool', description: 'CLI tool', 'allowed-tools': ['Bash', 'Read'] };
  ...
  it('reduces severity for well-structured frontmatter', () => {
    const manifest = { name: 'my-tool', description: 'A dev tool', license: 'MIT', metadata: { version: '1.0.0' } };
  ...
  it('reduces severity when dangerous patterns only in code blocks', () => {
    const content = 'This tool helps with deployment.\n```bash\nrm -rf dist/\n```\nThat clears old builds.';
...
Use \`rm -rf dist/\` to clean builds.`;
  ...
  it('multiplier is clamped between 0.3 and 2.5', () => {
    ...
    const minManifest = {
      name: 'tool', description: 'CLI automation tool', license: 'MIT',
      'allowed-tools': ['Bash'], metadata: { version: '1.0.0' },
    };

...
export class ThreatCloudClient {
   ...
   * 上傳匿名化威脅數據至雲端
   *
   * If offline or upload fails, data is queued locally for later sync.
   * 如果離線或上傳失敗，數據會被佇列在本地等待後續同步。
   *
  ...
  async upload(data: AnonymizedThreatData): Promise<boolean> {
    if (this.status === 'offline' || !this.endpoint) {
      ...
      this.saveQueue();
      logger.info('Data queued for later upload (offline mode) / 數據已佇列等待上傳');
      return false;
    }
    ...
    try {
      ...
      logger.info('Threat data uploaded successfully / 威脅數據上傳成功');
      return true;
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : String(err);
      logger.error(`Upload failed: ${msg}, queueing locally / 上傳失敗: ${msg}，本地佇列`);
      this.uploadQueue.push(data);
      this.saveQueue();

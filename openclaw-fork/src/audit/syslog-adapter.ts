 ...
 *
 * @module @openclaw/security-hardening/audit/syslog-adapter
 */
...
export function formatSyslogMessage(event: AuditEvent): string {
  ...
  const host = hostname();
  const appName = 'openclaw-security';
  const procId = process.pid;
  const msgId = event.action;
  const timestamp = event.timestamp;

  const structuredData = `[openclaw action="${event.action}" target="${event.target}" result="${event.result}"]`;
